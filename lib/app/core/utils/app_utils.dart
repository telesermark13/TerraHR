...
class AppUtils {
  ...
  static String getYearMonthFromDays(int days) {
    ...
    // removed unused 'week' local and prefer returning year/month only
    return "$year Year $month Months";
  }

  static bool isFirstBeforeOrEqualThanSecond(DateTime date1, DateTime date2) {
  ...
  static String getDateAndDay() {
    ...
    return '${json.decode(dayData)['${date.weekday}']}, ${json.decode(monthData)['${date.month}']} ${date.day}';
    // date.year.toString();
  }
  ...
  static String getReversedDate(String date) {
  ...
  static bool isTimePassed(String time) {
    ...
    DateTime expirationDate =
        DateFormat("yyyy-MM-dd HH:mm:ss").parse(todayDate);
  ...
  static Future<String> getFileSize(File file) async {
    ...
    return '${(bytes / mth.pow(1024, i)).toStringAsFixed(decimals)} ${suffixes[i]}';
  }

  static IconData getEditIcon(String key) {
    switch (key) {
      case 'Designation':
  ...
  static String getRoutedAction(String action) {
    switch (action) {
      ...
      //Leave
      case 'LV':
        return Routes.leaveStatusPage;
      //Regularization
      case 'RG':
        return Routes.regularizationDisplay;
      //Resignation
      case 'RN':
        return Routes.resinStatusPage;
      //Reimbursement
      case 'RM':
        return Routes.reimburseStatusPage;
      //Happy Birthday
      case 'HB':
        return Routes.splashPage;
      //General
